...
public class ItemsEditorWindow : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        if (!itemsDatabase)
        {
            EditorGUILayout.HelpBox("No ItemsDatabase found in the project." +
                                    " You had one job...", MessageType.Error);
            return;
        }
    ...
    private void PopulateSearchResults()
    {
        ...
        foreach (var item in itemsDatabase.items)
        {
            if (item && item.name.ToLower().Contains(searchQuery.ToLower()))
            {
                ...
                if (item.itemSprite)
                {
                    GUILayout.Label(item.itemSprite.texture, GUILayout.Width(40), GUILayout.Height(40));
                }
